...
test('task editor keeps a concise title and optional next step without unsafe characters', () => {
    ...
    const patch = taskEditorPatch({
        task: { bucket: 'inbox' }, title: '  整理资料  ', note: '  先找三份参考  ', microSteps: ['打开资料库', '列出参考'], bucket: 'later',
    }, new Date(2026, 6, 18, 9));
    assert.deepEqual(patch, {
        title: '整理资料', note: '先找三份参考', microSteps: [
            { id: 'micro-1', text: '打开资料库', completed: false },
            { id: 'micro-2', text: '列出参考', completed: false },
...
test('task editor moves a task into today, retains its current soft block, and rejects empty titles', () => {
    ...
    const patch = taskEditorPatch({
        task: { bucket: 'today', timeBlock: 'morning' }, title: '收尾邮件', note: '', microSteps: [], bucket: 'today',
    }, now);
