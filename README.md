...
v2 在 v1 的动画宠物之上，增加了**性格系统**（心情 / 好感 / 记忆）、**效率工具**（番茄钟 / 待办 / 提醒 / 勿扰）、**成就与换装**、**本地对话 + 可选 LLM 接入**，以及一个独立的**角色房间窗口**。所有状态通过磁盘持久化（8 个数据域）。

## 特性

- 🎨 **18 个状态**：12 个已有立绘 + 6 个占位状态（无立绘时回退到近似立绘，见下表）
...
- ⏱ **番茄钟**：工作 / 短休 / 长休阶段，气泡提示
- ☑️ **待办清单**：支持优先级、每日 / 每周重复
- 🔔 **提醒引擎**：内置喝水 / 久坐 / 护眼三类，可自定义，可贪睡 5 分钟
- 🌙 **勿扰模式**：手动或按时间段自动，静音 + 抑制提醒
- 💬 **安全 AI 接入**：本地对话库始终可用；可选 OpenAI 兼容接口由主进程代理，密钥使用 Windows 安全存储加密
- 👗 **换装（Wardrobe）**：预留多套立绘目录（`assets/outfits/<name>/`）
- 🏠 **角色房间窗口**：状态 / 成就 / 换装 / 投喂 / 设置 5 个标签页
- 💾 **安全备份与迁移**：设置页可导出 / 导入八个数据域；导入前执行格式、类型、范围和大小校验
...
6 个占位状态（`hasSprite: false`，暂无专属立绘、默认在菜单中隐藏，可在设置里开启"显示未完成动作"）：
...
> `Esc` 回到 IDLE。占位状态在拿到正式立绘后，把目录中的 `hasSprite` 翻为 `true` 即可自动上线。
...
| 域 | 内容 |
|----|------|
| `settings` | 音量、活跃度、提醒开关与间隔、勿扰、换装、开机启动、自动更新开关、AI 服务地址与模型等；不含 API 密钥 |
...
设置页的“数据与备份”可导出单个版本化 JSON 文件。导入时要求八个数据域完整，文件不超过 2 MB；只有在全部校验通过并经用户确认后才会替换数据，替换前仍会保留当前滚动备份。
...
- API 密钥不属于八个普通数据域，不会进入设置 JSON、导出的备份或 Web Storage。点击“清除远程配置”会同时删除密钥文件。
...
- **新状态不显示**：占位状态默认隐藏，勾选设置里的"显示未完成动作"；补齐立绘后把 `hasSprite` 翻为 `true` 并重跑 `npm run manifest`
